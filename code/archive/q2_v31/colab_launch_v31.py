"""Launch V3.1 notebook execution in a detached Colab worker."""
from pathlib import Path
import json,os,subprocess,sys,textwrap,time
P=Path('/content/CUMCM_2026_Last_Dance');S=Path('/content/q2_v31_status.json');L=Path('/content/q2_v31_worker.log')
worker=r'''
from pathlib import Path
...
try:
    ...
    with L.open('a') as log:
        fit=subprocess.run([sys.executable,'-m','jupyter','nbconvert','--to','notebook','--execute','code/problem2_v31_terminal_ablation.ipynb',
          '--output','problem2_v31_terminal_ablation_output.ipynb','--output-dir','code','--ExecutePreprocessor.timeout=43200'],stdout=log,stderr=log,text=True)
    if fit.returncode:raise RuntimeError(f'nbconvert return code {fit.returncode}')
    status('packing_outputs')
    targets=[P/'code/problem2_v31_terminal_ablation_output.ipynb',P/'results/q2_v31_terminal_ablation',
             P/'figures/q2_v31_terminal_ablation',P/'reports/Q2_V31_TERMINAL_ABLATION_RESULTS.md']
    missing=[str(x) for x in targets if not x.exists()]
    if missing:raise FileNotFoundError(missing)
    archive=Path('/content/q2_v31_outputs.tar.gz')
...
'''
path=Path('/content/q2_v31_worker.py');path.write_text(textwrap.dedent(worker),encoding='utf-8')
