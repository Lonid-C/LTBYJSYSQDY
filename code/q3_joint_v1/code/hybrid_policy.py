...
def run_stoch(L, P, price, bank, rbank, p, pol: Policy, tv: TerminalValue,
              mask=(6, 12, 18), start=31, end=365, initial=6000., detail=False, seed=0, scenario_provider=None):
    ...
    def scen(day, k):
        if scenario_provider is not None:
            return scenario_provider(day, k)
...
def run_stoch2(L, P, price, bank, rbank, p, pol: Policy, tv: TerminalValue,
    ...
    for day in range(start, end):
        begin = soc
        nets, w = scen(day, 0)
        pred = HS.solve_stoch2(nets, w, price, soc, p, tv_use, None, 0, next_lock(0, mask),
        ...
        for k, h in enumerate(ISSUES):
            ...
            if h in mask:
                ...
                priceH = np.r_[price[t:], price[:t]]
                base = q0[t:]
                cand = HS.solve_stoch2(netsk, wk, priceH, soc, p, tv_use, base, tail, next_lock(h, mask),
                                       emergency_scale=esc, cvar_beta=pol.cvar_beta, cvar_alpha=pol.cvar_alpha,
                                       adj_premium=pol.adj_premium)
                if cand is None:
                    raise RuntimeError('adjustment infeasible')
...
def next_lock(hour, mask):
    """Slots until next permitted release; midnight always opens a new plan."""
    return 6 * (min([h for h in mask if h > hour] + [24]) - hour)
