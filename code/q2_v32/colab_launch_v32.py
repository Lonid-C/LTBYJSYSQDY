...
S=Path('/content/q2_v32_status.json');L=Path('/content/q2_v32_worker.log')
worker=r'''
from pathlib import Path
import json,os,subprocess,sys,time,traceback,tarfile
P=Path('/content/CUMCM_2026_Last_Dance');S=Path('/content/q2_v32_status.json');L=Path('/content/q2_v32_worker.log')
...
try:
    ...
    with L.open('a') as log:
        fit=subprocess.run([sys.executable,'-m','jupyter','nbconvert','--to','notebook','--execute','code/problem2_v32_final.ipynb',
          '--output','problem2_v32_final_output.ipynb','--output-dir','code','--ExecutePreprocessor.timeout=43200'],stdout=log,stderr=log,text=True)
    if fit.returncode:raise RuntimeError(f'nbconvert return code {fit.returncode}')
    status('packing_outputs')
    targets=[P/'code/problem2_v32_final_output.ipynb',P/'results/q2_v32_final',P/'result2.xlsx',
             P/'reports/Q2_V32_FINAL_RESULTS.md']
    missing=[str(x) for x in targets if not x.exists()]
    if missing:raise FileNotFoundError(missing)
    archive=Path('/content/q2_v32_outputs.tar.gz')
...
'''
path=Path('/content/q2_v32_worker.py');path.write_text(textwrap.dedent(worker),encoding='utf-8')
