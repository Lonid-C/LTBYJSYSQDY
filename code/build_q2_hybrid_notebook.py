...
统计参数不是随意给定：残差窗口 14/21/28/42/56 天对应 2/3/4/6/8 个完整周；$\alpha$ 先在 $[0.50,0.95]$ 上以 0.05 粗搜，再在局部以 0.01 细搜；$\rho\in\{0,0.25,0.5,0.75,1\}$ 覆盖完全灵活到完全跟随参考库存；0/3/5 槽平滑用于检验局部分位数噪声。每月配置仅由月初之前 56 个已结束日期选择，11—12 月冻结 10 月规则。
